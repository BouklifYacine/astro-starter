...
2. Analyze product pages using `.claude/skills/seo/scripts/render_page.py --mode auto` and `.claude/skills/seo/scripts/parse_html.py`
...
4. If DataForSEO credentials available, fetch marketplace data via
   `.claude/skills/seo/scripts/dataforseo_merchant.py`

## Cost Guardrails

Before ANY DataForSEO Merchant API call:
```bash
python .claude/skills/seo/scripts/runtime.py run dataforseo_costs.py check <endpoint>
```
...
```bash
python .claude/skills/seo/scripts/runtime.py run dataforseo_costs.py log <endpoint> <actual_cost>
```
