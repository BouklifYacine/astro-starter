...
- `python .claude/skills/seo/scripts/runtime.py run drift_baseline.py <url>` -- capture a new baseline
- `python .claude/skills/seo/scripts/runtime.py run drift_compare.py <url>` -- compare current state to baseline
- `python .claude/skills/seo/scripts/runtime.py run drift_history.py <url>` -- show change history
- `python .claude/skills/seo/scripts/runtime.py run drift_report.py <file> --output report.html` -- generate HTML report

Never use curl, wget, or raw HTTP requests. All fetching is handled by
`.claude/skills/seo/scripts/fetch_page.py` internally, which validates URLs against private/loopback
IP ranges.
