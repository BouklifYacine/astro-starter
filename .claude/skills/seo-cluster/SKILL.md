...
**Scripts:** Located at the plugin root `.claude/skills/seo/scripts/` directory.
...
instead of WebSearch for SERP data. Run `python .claude/skills/seo/scripts/runtime.py run dataforseo_costs.py check serp_organic_live_advanced --count N`
...
- All URLs fetched via `python .claude/skills/seo/scripts/runtime.py run render_page.py <url> --mode auto` (SPA-aware SSRF protection via `url_safety`)
