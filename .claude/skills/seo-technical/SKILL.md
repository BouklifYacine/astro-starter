...
- XML sitemap: run `python .claude/skills/seo/scripts/runtime.py run sitemap_discovery.py <url> --json`; require a
...
python .claude/skills/seo/scripts/runtime.py run agent_ux_check.py https://example.com --json
```
...
without scoring, use `python .claude/skills/seo/scripts/runtime.py run render_page.py <url> --a11y-tree --json`.
...
If Google API credentials are configured, use `python .claude/skills/seo/scripts/runtime.py run pagespeed_check.py <url> --json` for real PSI + CrUX field data (replaces lab-only CWV estimates), `python .claude/skills/seo/scripts/runtime.py run crux_history.py <url> --json` for 25-week CWV trends, and `python .claude/skills/seo/scripts/runtime.py run gsc_inspect.py <url> --json` for real indexation status per URL.
