...
1. Fetch the target URL via `.claude/skills/seo/scripts/render_page.py --mode auto` (SPA-aware and SSRF-safe)
2. Parse with `.claude/skills/seo/scripts/parse_html.py` to extract: title, H1, meta description,
...
- [ ] Target URL was fetched via `.claude/skills/seo/scripts/render_page.py --mode auto` (not raw curl/fetch)
