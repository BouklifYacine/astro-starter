...
**Runtime:** Run bundled Python tools through `python .claude/skills/seo/scripts/runtime.py run <script.py>`. Plugin
...
3. If Google API credentials detected (`python .claude/skills/seo/scripts/runtime.py run google_auth.py --check`), also spawn seo-google agent
...
6. If backlink APIs detected (`python .claude/skills/seo/scripts/runtime.py run backlinks_auth.py --check`), also spawn seo-backlinks agent
...
10. If drift baseline exists for this URL (`python .claude/skills/seo/scripts/runtime.py run drift_history.py <url>`), also spawn seo-drift agent
...
After any analysis command completes, offer to generate a PDF report via `.claude/skills/seo/scripts/google_report.py`.
