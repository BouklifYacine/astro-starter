...
try:
    ...
    raise RuntimeError(
        ".claude/skills/seo/scripts/url_safety.py is required for SSRF protection. "
    ...
    if service == "moz":
        ...
        if api_key:
            ...
            result["note"] = (
                ...
                "Run a Moz command such as `python .claude/skills/seo/scripts/moz_api.py metrics "
...
-------------------------------------------------
  ...
  Note: `--check moz` reports whether credentials are configured; run
        `python .claude/skills/seo/scripts/moz_api.py metrics example.com --json` for a live
...
VERIFY CONFIGURATION:
  python .claude/skills/seo/scripts/backlinks_auth.py --check
  python .claude/skills/seo/scripts/backlinks_auth.py --tier
