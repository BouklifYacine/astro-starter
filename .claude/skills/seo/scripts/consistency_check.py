...
3. ``.claude/skills/seo/scripts/X.py`` mentions resolve PATH-AWARE: repo-root ``.claude/skills/seo/scripts/`` first, then the
   enclosing extension's ``.claude/skills/seo/scripts/`` dir for files under ``extensions/<name>/``. A bare
   basename existing somewhere else in the tree does NOT count (this exact bug hid dead
   ``.claude/skills/seo/scripts/presets.py`` invocations before the 2026-07 full review).
...
Usage:
    python3 .claude/skills/seo/scripts/consistency_check.py [--json] [--strict]
...
SELF_DOC = {".claude/skills/seo/scripts/consistency_check.py", "tests/test_consistency_check.py"}
...
def check_script_refs(files, texts):
    """Path-aware: .claude/skills/seo/scripts/X.py must exist at repo .claude/skills/seo/scripts/ or the enclosing extension's.

    The lookbehind skips path-qualified mentions such as
    ``extensions/banana/.claude/skills/seo/scripts/edit.py`` (checked via their own full path elsewhere).
    ``research/`` notes, ``CHANGELOG.md``, and ``.github/`` templates are
    historical/example carriers.
    """
    pat = re.compile(r'(?<![\w/]).claude/skills/seo/scripts/([A-Za-z0-9_]+\.py)\b')
    errors = []
    mentioned = set()
    for f in texts:
        ...
        for m in sorted(set(pat.findall(read(f)))):
            mentioned.add(m)
            candidates = [f".claude/skills/seo/scripts/{m}"]
            if ext_root:
                candidates.insert(0, f"{ext_root}/.claude/skills/seo/scripts/{m}")
            if not any(c in files or os.path.isfile(os.path.join(REPO, c)) for c in candidates):
                errors.append(f"{f}: dead script ref .claude/skills/seo/scripts/{m} "
                              f"(checked: {', '.join(candidates)})")
    orphans = []
    all_text = "\n".join(read(f) for f in texts)
    for s in sorted(os.path.basename(f) for f in files
                    if f.startswith(".claude/skills/seo/scripts/") and f.endswith(".py")):
        if s in ORPHAN_SCRIPT_WHITELIST:
            continue
        others = all_text.count(s) - read(f".claude/skills/seo/scripts/{s}").count(s)
        if others <= 0:
            orphans.append(f".claude/skills/seo/scripts/{s}: referenced nowhere outside itself")
...
def check_skill_dir_script_refs(files, texts):
    ...
    """
    pat = re.compile(r'\$\{CLAUDE_SKILL_DIR\}/.claude/skills/seo/scripts/([A-Za-z0-9_]+\.py)\b')
    errors = []
    for f in texts:
        for script in sorted(set(pat.findall(read(f)))):
            ext_root = extension_root(f)
            if ext_root:
                candidate = f"{ext_root}/.claude/skills/seo/scripts/{script}"
            elif f.startswith("skills/"):
                skill_root = "/".join(f.split("/")[:2])
                candidate = f"{skill_root}/.claude/skills/seo/scripts/{script}"
            else:
...
def check_runtime_invocations(texts):
    ...
    bare = re.compile(
        r"\b(?:python3|python|py\s+-3)\s+[^\n`]*?.claude/skills/seo/scripts/[A-Za-z0-9_./-]+\.py"
    )
