...
try:
    ...
    print("Error: backlinks_auth.py and google_auth.py required in .claude/skills/seo/scripts/", file=sys.stderr)
    sys.exit(1)
...
def main():
    ...
    if not api_key:
        result = {
            "status": "error",
            "data": None,
            "error": "No Bing Webmaster API key configured. Run: python .claude/skills/seo/scripts/runtime.py run backlinks_auth.py --setup",
            "metadata": {"source": "bing_webmaster"},
        }
        if args.json:
            print(json.dumps(result, indent=2))
