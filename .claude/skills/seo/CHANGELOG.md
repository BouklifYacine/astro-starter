...
- GBP Q&A false-deprecation removed from `.claude/skills/seo/scripts/gbp_deprecation_lint.py` and
...
- Stale Gemini image-model IDs refreshed across seo-image-gen and the banana extension;
  dead `.claude/skills/seo/scripts/presets.py` / `cost_tracker.py` invocations repointed.
...
- `.claude/skills/seo/scripts/consistency_check.py`: path-aware reference-graph gate (dead refs, routing tables,
...
- `.claude/skills/seo/scripts/iptc_ai_label.py`: `algorithmicMedia` and `compositeWithTrainedAlgorithmicMedia` DigitalSourceType values.
...
- **CONTRIBUTING.md:** corrected the SSRF guidance — fetchers route through `.claude/skills/seo/scripts/url_safety.py` (`validate_url()` / `safe_requests_session()`), not `google_auth.py` (OAuth-only).
...
- Removed the duplicate root `CODEOWNERS`; CI compiles every `.claude/skills/seo/scripts/*.py` dynamically; marketplace extension count corrected from 7 to 8; dependency floor bumps (Dependabot #105 to #109, #116).
...
- **Foundation:** `.claude/skills/seo/scripts/url_safety.py` (canonical SSRF + DNS-rebinding module, 83 test cases), `.claude/skills/seo/scripts/render_page.py` (shared Playwright Chromium renderer with `--mode auto` SPA detection, trafilatura extraction, htmldate publication-date extraction, 27 test cases). 8 fetcher subagents (seo-technical, seo-content, seo-schema, seo-geo, seo-local, seo-ecommerce, seo-backlinks, seo-sxo) now route through `render_page`.
...
- `.claude/skills/seo/scripts/google_auth.py:validate_url` now delegates to `url_safety.validate_url`. Strict variant available as `url_safety.validate_url_strict`.
- `.claude/skills/seo/scripts/fetch_page.py` exposes `--render {auto,always,never}`. Default `auto` runs raw fetch then renders if SPA signals detected; `always` forces Playwright; `never` preserves v1 behaviour.
- `.claude/skills/seo/scripts/capture_screenshot.py` uses `url_safety.make_safe_playwright_route_handler` as defense-in-depth against subresource SSRF (data: allowed, private resolutions aborted, AF_UNSPEC IPv6-aware).
...
1. `.claude/skills/seo/scripts/backlinks_auth.py` no longer ships a silent SSRF-disabled fallback. If `url_safety` cannot be imported, the module raises `RuntimeError` at import time.
...
- **23 in-tree skill `metadata.version` fields** were stuck at `1.9.6`; 3
  ...
  CI allowlist).
- **Image audit (issue #41)**: `.claude/skills/seo/scripts/parse_html.py` now classifies each
...
- **`.claude/skills/seo/scripts/sync_flow.py`**: GitHub API sync script, pulls latest FLOW prompts, framework doc, and bibliography from AgriciDaniel/flow. Supports `--dry-run` and `--ref <sha>` pinning. Outputs JSON summary.
...
- **DataForSEO cost guardrails**: `.claude/skills/seo/scripts/dataforseo_costs.py` with threshold-based approval, session budget tracking, daily spend summaries. `references/cost-tiers.md` pricing table. Contributed by Matej Marjanovic.
...
- **Excel export**: `--format xlsx` option in `.claude/skills/seo/scripts/google_report.py`
...
- **PDF report generator**: `.claude/skills/seo/scripts/google_report.py` with enterprise A4 template, WeasyPrint + matplotlib charts, post-generation quality review
...
- Standard directory layout: `.claude/skills/seo/scripts/`, `references/`, `assets/`
