...
**Scripts:** Located at the plugin root `.agents/skills/seo/scripts/` directory.
...
3. If Google API credentials detected (`python .agents/skills/seo/scripts/google_auth.py --check`), also spawn seo-google agent
...
6. If backlink APIs detected (`python .agents/skills/seo/scripts/backlinks_auth.py --check`), also spawn seo-backlinks agent
...
10. If drift baseline exists for this URL (`python .agents/skills/seo/scripts/drift_history.py <url>`), also spawn seo-drift agent
...
After any analysis command completes, offer to generate a PDF report via `.agents/skills/seo/scripts/google_report.py`.
