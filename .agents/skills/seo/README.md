...
flowchart LR
  ...
  skills --> scripts[".agents/skills/seo/scripts/<br/>deterministic runners"]
...
- `.agents/skills/seo/scripts/run_skill_workflow.py` standardizes output for every user-invokable workflow.
- `.agents/skills/seo/scripts/run_api_smoke_suite.py` runs all supported workflows in one pass.
...
| Google APIs | `seo-google`, `seo-performance` | `python .agents/skills/seo/scripts/google_auth.py --setup` | PageSpeed, CrUX, GSC, URL Inspection, Indexing API, GA4 |
...
```bash
python .agents/skills/seo/scripts/demo_readiness.py --target https://example.com --live-apis --workflows --json
```

One low-depth DataForSEO proof:

```bash
python .agents/skills/seo/scripts/demo_readiness.py --target https://example.com --live-apis --live-serp --serp-keyword "seo tools" --json
```
...
```bash
python .agents/skills/seo/scripts/run_skill_workflow.py --skill seo-technical https://example.com --json
python .agents/skills/seo/scripts/run_skill_workflow.py --skill seo-google https://example.com --json
python .agents/skills/seo/scripts/run_skill_workflow.py --skill seo-dataforseo https://example.com --json
```

Run the full smoke suite:

```bash
python .agents/skills/seo/scripts/run_api_smoke_suite.py https://example.com --json
```

Verify environment:

```bash
python .agents/skills/seo/scripts/verify_environment.py --target https://example.com --json
```

Bootstrap a clean runtime:

```bash
python .agents/skills/seo/scripts/bootstrap_environment.py --venv .venv --json
```
...
flowchart TB
  ...
  scriptsDir[".agents/skills/seo/scripts/<br/>deterministic runners"] --> specialists
...
├── .agents/skills/seo/scripts/                         # Deterministic runners and API helpers
├── extensions/
...
- DataForSEO calls use cost guardrails through `.agents/skills/seo/scripts/dataforseo_costs.py`.
