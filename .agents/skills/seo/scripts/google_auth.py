...
def run_oauth_flow(creds_path: str):
    ...
    if not auth_code[0]:
        ...
        print(f"  python .agents/skills/seo/scripts/google_auth.py --exchange --creds {creds_path} --code 'THE_CODE'")
        sys.exit(1)
        ...
        if token_data and token_data.get("access_token"):
            ...
            if not sa_path:
                result["error"] = (
                    ...
                    "         1. Run: python .agents/skills/seo/scripts/google_auth.py --auth --creds /path/to/client_secret.json\n"
...
7. VERIFY
   python .agents/skills/seo/scripts/google_auth.py --check
