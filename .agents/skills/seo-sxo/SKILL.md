...
1. Fetch the target URL via `.agents/skills/seo/scripts/fetch_page.py` (SSRF-safe)
2. Parse with `.agents/skills/seo/scripts/parse_html.py` to extract: title, H1, meta description,
...
- [ ] Target URL was fetched via `.agents/skills/seo/scripts/fetch_page.py` (not raw curl/fetch)
