...
1. **Fetch homepage**: use `.agents/skills/seo/scripts/fetch_page.py` to retrieve HTML
...
4. **Delegate to subagents** (if available, otherwise run inline sequentially):
   ...
   - `seo-google` -- CWV field data (CrUX), URL indexation (GSC), organic traffic (GA4) (spawn when Google API credentials detected via `python .agents/skills/seo/scripts/google_auth.py --check`)
   - `seo-backlinks` -- Backlink profile data: DA/PA, referring domains, anchor text, toxic links (spawn when Moz or Bing API credentials detected via `python .agents/skills/seo/scripts/backlinks_auth.py --check`, or always include Common Crawl domain-level metrics)
   ...
   - `seo-drift` -- Drift analysis: compare against stored baseline (spawn when drift baseline exists for the URL via `python .agents/skills/seo/scripts/drift_history.py <url>`)
...
- **PDF Report** (recommended): Generate a professional A4 PDF using `.agents/skills/seo/scripts/google_report.py --type full`. This produces a white-cover enterprise report with TOC, executive summary, charts (Lighthouse gauges, query bars, index donut), metric cards, threshold tables, prioritized recommendations with effort estimates, and implementation roadmap. Always offer PDF generation after completing an audit.
