...
If Google API credentials are configured, use `python .agents/skills/seo/scripts/pagespeed_check.py <url> --json` for real PSI + CrUX field data (replaces lab-only CWV estimates), `python .agents/skills/seo/scripts/crux_history.py <url> --json` for 25-week CWV trends, and `python .agents/skills/seo/scripts/gsc_inspect.py <url> --json` for real indexation status per URL.
